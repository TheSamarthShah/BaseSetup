...
export class ColumnMetadataService {
  private apiUrl = `${API_ENDPOINTS.BASE + API_ENDPOINTS.CORE.COLUMNMETADATA.GET}`;
  ...
  private loadMetadata(): void {
    ...
    this.http.post<ColumnMetadata[]>(this.apiUrl, {}) // Add request body if required
      .pipe(
        tap(data => {
          this.metadataCache$.next(data);
        }),
        catchError(err => {
